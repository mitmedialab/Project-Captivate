...
/* COAP */
typedef void (*CoapRequestHandlerCallback)(otCoapHeader *aHeader,
		otMessage *aMessage, const otMessageInfo *aMessageInfo);
typedef void (*CoapResponseHandlerCallback)(otCoapHeader *aHeader,
...
HAL_StatusTypeDef OpenThread_CallBack_Processing(void) {
	...
	Thread_OT_Cmd_Request_t *p_notification =
			THREAD_Get_NotificationPayloadBuffer();
	...
	case MSG_M0TOM4_COAP_REQUEST_HANDLER:
		mySTCoapContext =
				(STCoapSpecificRequestContextType*) p_notification->Data[0];
	...
	case MSG_M0TOM4_COAP_RESPONSE_HANDLER:
		mySTCoapResponseContext =
				(STCoapSpecificResponseContextType*) p_notification->Data[0];
	...
	case MSG_M0TOM4_HANDLE_ACTIVE_SCAN_RESULT:
		if (otHandleActiveScanResultCb != NULL) {
			otHandleActiveScanResultCb(
					(otActiveScanResult*) p_notification->Data[0],
					(void*) p_notification->Data[1]);
		}
		break;
	case MSG_M0TOM4_HANDLE_ENERGY_SCAN_RESULT:
		if (otHandleEnergyScanResultCb != NULL) {
			otHandleEnergyScanResultCb(
					(otEnergyScanResult*) p_notification->Data[0],
					(void*) p_notification->Data[1]);
		}
		break;
	...
	case MSG_M0TOM4_THREAD_FTD_CHILD_TABLE_CALLBACK:
		if (otThreadChildTableCallbackCb != NULL) {
			otThreadChildTableCallbackCb(
					(otThreadChildTableEvent) p_notification->Data[0],
