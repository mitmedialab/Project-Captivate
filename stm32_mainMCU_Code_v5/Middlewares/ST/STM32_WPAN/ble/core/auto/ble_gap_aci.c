...
tBleStatus aci_gap_set_limited_discoverable(uint8_t Advertising_Type,
	...
	aci_gap_set_limited_discoverable_cp1 *cp1 =
			(aci_gap_set_limited_discoverable_cp1*) (cmd_buffer + 1 + 2 + 2 + 1
					+ 1 + 1 + Local_Name_Length * (sizeof(uint8_t)));
	aci_gap_set_limited_discoverable_cp2 *cp2 =
			(aci_gap_set_limited_discoverable_cp2*) (cmd_buffer + 1 + 2 + 2 + 1
					+ 1 + 1 + Local_Name_Length * (sizeof(uint8_t)) + 1
...
tBleStatus aci_gap_set_direct_connectable(uint8_t Own_Address_Type,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	aci_gap_set_direct_connectable_cp0 *cp0 =
			(aci_gap_set_direct_connectable_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
	cp0->Own_Address_Type = Own_Address_Type;
	index_input += 1;
...
tBleStatus aci_gap_authorization_resp(uint16_t Connection_Handle,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	aci_gap_authorization_resp_cp0 *cp0 =
			(aci_gap_authorization_resp_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
	cp0->Connection_Handle = Connection_Handle;
	index_input += 2;
...
tBleStatus aci_gap_set_non_connectable(uint8_t Advertising_Event_Type,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	aci_gap_set_non_connectable_cp0 *cp0 =
			(aci_gap_set_non_connectable_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
...
tBleStatus aci_gap_slave_security_req(uint16_t Connection_Handle) {
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	aci_gap_slave_security_req_cp0 *cp0 =
			(aci_gap_slave_security_req_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
	cp0->Connection_Handle = Connection_Handle;
	index_input += 2;
...
tBleStatus aci_gap_get_security_level(uint16_t Connection_Handle,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	aci_gap_get_security_level_cp0 *cp0 =
			(aci_gap_get_security_level_cp0*) (cmd_buffer);
...
tBleStatus aci_gap_start_name_discovery_proc(uint16_t LE_Scan_Interval,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	aci_gap_start_name_discovery_proc_cp0 *cp0 =
			(aci_gap_start_name_discovery_proc_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
	cp0->LE_Scan_Interval = LE_Scan_Interval;
	index_input += 2;
...
tBleStatus aci_gap_start_auto_connection_establish_proc(
	...
	index_input += 1;
	Osal_MemCpy((void*) &cp0->Whitelist_Entry, (const void*) Whitelist_Entry,
			Num_of_Whitelist_Entries * (sizeof(Whitelist_Entry_t)));
...
tBleStatus aci_gap_create_connection(uint16_t LE_Scan_Interval,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	aci_gap_create_connection_cp0 *cp0 =
			(aci_gap_create_connection_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
	cp0->LE_Scan_Interval = LE_Scan_Interval;
	index_input += 2;
...
tBleStatus aci_gap_terminate_gap_proc(uint8_t Procedure_Code) {
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	aci_gap_terminate_gap_proc_cp0 *cp0 =
			(aci_gap_terminate_gap_proc_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
...
tBleStatus aci_gap_start_connection_update(uint16_t Connection_Handle,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	aci_gap_start_connection_update_cp0 *cp0 =
			(aci_gap_start_connection_update_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
	cp0->Connection_Handle = Connection_Handle;
	index_input += 2;
...
tBleStatus aci_gap_send_pairing_req(uint16_t Connection_Handle,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	aci_gap_send_pairing_req_cp0 *cp0 =
			(aci_gap_send_pairing_req_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
	cp0->Connection_Handle = Connection_Handle;
	index_input += 2;
...
tBleStatus aci_gap_resolve_private_addr(const uint8_t *Address,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	aci_gap_resolve_private_addr_cp0 *cp0 =
			(aci_gap_resolve_private_addr_cp0*) (cmd_buffer);
...
tBleStatus aci_gap_set_broadcast_mode(uint16_t Advertising_Interval_Min,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	aci_gap_set_broadcast_mode_cp0 *cp0 =
			(aci_gap_set_broadcast_mode_cp0*) (cmd_buffer);
	...
	/* var_len_data input */
	{
		Osal_MemCpy((void*) &cp0->Adv_Data, (const void*) Adv_Data,
				Adv_Data_Length);
		...
		index_input += 1;
		Osal_MemCpy((void*) &cp1->Whitelist_Entry,
				(const void*) Whitelist_Entry,
				Num_of_Whitelist_Entries * (sizeof(Whitelist_Entry_t)));
		index_input += Num_of_Whitelist_Entries * (sizeof(Whitelist_Entry_t));
	}
	Osal_MemSet(&rq, 0, sizeof(rq));
...
tBleStatus aci_gap_start_observation_proc(uint16_t LE_Scan_Interval,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	aci_gap_start_observation_proc_cp0 *cp0 =
			(aci_gap_start_observation_proc_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
	cp0->LE_Scan_Interval = LE_Scan_Interval;
	index_input += 2;
...
tBleStatus aci_gap_get_bonded_devices(uint8_t *Num_of_Addresses,
	...
	*Num_of_Addresses = resp.Num_of_Addresses;
	Osal_MemCpy((void*) Bonded_Device_Entry,
...
tBleStatus aci_gap_is_device_bonded(uint8_t Peer_Address_Type,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	aci_gap_is_device_bonded_cp0 *cp0 =
			(aci_gap_is_device_bonded_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
	cp0->Peer_Address_Type = Peer_Address_Type;
	index_input += 1;
...
tBleStatus aci_gap_add_devices_to_resolving_list(
	...
	/* var_len_data input */
	{
		Osal_MemCpy((void*) &cp0->Whitelist_Identity_Entry,
				(const void*) Whitelist_Identity_Entry,
				Num_of_Resolving_list_Entries
						* (sizeof(Whitelist_Identity_Entry_t)));
...
tBleStatus aci_gap_remove_bonded_device(uint8_t Peer_Identity_Address_Type,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	aci_gap_remove_bonded_device_cp0 *cp0 =
			(aci_gap_remove_bonded_device_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
