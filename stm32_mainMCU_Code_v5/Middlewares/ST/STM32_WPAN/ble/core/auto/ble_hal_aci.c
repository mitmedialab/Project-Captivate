...
tBleStatus aci_hal_read_config_data(uint8_t Offset, uint8_t *Data_Length,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	aci_hal_read_config_data_cp0 *cp0 =
			(aci_hal_read_config_data_cp0*) (cmd_buffer);
...
tBleStatus aci_hal_set_smp_eng_config(uint32_t SMP_Config) {
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	aci_hal_set_smp_eng_config_cp0 *cp0 =
			(aci_hal_set_smp_eng_config_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
...
tBleStatus aci_hal_write_radio_reg(uint8_t Register_Address,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	aci_hal_write_radio_reg_cp0 *cp0 =
			(aci_hal_write_radio_reg_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
	cp0->Register_Address = Register_Address;
	index_input += 1;
