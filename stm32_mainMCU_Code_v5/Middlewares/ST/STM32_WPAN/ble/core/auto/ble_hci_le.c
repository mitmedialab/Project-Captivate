...
tBleStatus hci_read_transmit_power_level(uint16_t Connection_Handle,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	hci_read_transmit_power_level_cp0 *cp0 =
			(hci_read_transmit_power_level_cp0*) (cmd_buffer);
...
tBleStatus hci_read_local_supported_commands(uint8_t *Supported_Commands) {
	...
	if (resp.Status)
		return resp.Status;
	Osal_MemCpy((void*) Supported_Commands,
			(const void*) resp.Supported_Commands, 64);
	return BLE_STATUS_SUCCESS;
}
...
tBleStatus hci_le_set_random_address(const uint8_t *Random_Address) {
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	hci_le_set_random_address_cp0 *cp0 =
			(hci_le_set_random_address_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
...
tBleStatus hci_le_set_advertising_data(uint8_t Advertising_Data_Length,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	hci_le_set_advertising_data_cp0 *cp0 =
			(hci_le_set_advertising_data_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
...
tBleStatus hci_le_set_scan_response_data(uint8_t Scan_Response_Data_Length,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	hci_le_set_scan_response_data_cp0 *cp0 =
			(hci_le_set_scan_response_data_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
	cp0->Scan_Response_Data_Length = Scan_Response_Data_Length;
	index_input += 1;
	Osal_MemCpy((void*) &cp0->Scan_Response_Data,
			(const void*) Scan_Response_Data, 31);
	index_input += 31;
	Osal_MemSet(&rq, 0, sizeof(rq));
	rq.ogf = 0x08;
	rq.ocf = 0x009;
...
tBleStatus hci_le_set_advertise_enable(uint8_t Advertising_Enable) {
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	hci_le_set_advertise_enable_cp0 *cp0 =
			(hci_le_set_advertise_enable_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
...
tBleStatus hci_le_set_scan_parameters(uint8_t LE_Scan_Type,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	hci_le_set_scan_parameters_cp0 *cp0 =
			(hci_le_set_scan_parameters_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
...
tBleStatus hci_le_create_connection(uint16_t LE_Scan_Interval,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	hci_le_create_connection_cp0 *cp0 =
			(hci_le_create_connection_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
	cp0->LE_Scan_Interval = LE_Scan_Interval;
	index_input += 2;
...
tBleStatus hci_le_add_device_to_white_list(uint8_t Address_Type,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	hci_le_add_device_to_white_list_cp0 *cp0 =
			(hci_le_add_device_to_white_list_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
...
tBleStatus hci_le_remove_device_from_white_list(uint8_t Address_Type,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	hci_le_remove_device_from_white_list_cp0 *cp0 =
			(hci_le_remove_device_from_white_list_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
	cp0->Address_Type = Address_Type;
	index_input += 1;
...
tBleStatus hci_le_connection_update(uint16_t Connection_Handle,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	hci_le_connection_update_cp0 *cp0 =
			(hci_le_connection_update_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
	cp0->Connection_Handle = Connection_Handle;
	index_input += 2;
...
tBleStatus hci_le_read_channel_map(uint16_t Connection_Handle,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	hci_le_read_channel_map_cp0 *cp0 =
			(hci_le_read_channel_map_cp0*) (cmd_buffer);
...
tBleStatus hci_le_read_remote_features(uint16_t Connection_Handle) {
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	hci_le_read_remote_features_cp0 *cp0 =
			(hci_le_read_remote_features_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
	cp0->Connection_Handle = Connection_Handle;
	index_input += 2;
...
tBleStatus hci_le_start_encryption(uint16_t Connection_Handle,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	hci_le_start_encryption_cp0 *cp0 =
			(hci_le_start_encryption_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
	cp0->Connection_Handle = Connection_Handle;
	index_input += 2;
...
tBleStatus hci_le_transmitter_test(uint8_t TX_Frequency,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	hci_le_transmitter_test_cp0 *cp0 =
			(hci_le_transmitter_test_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
...
tBleStatus hci_le_generate_dhkey(const uint8_t *Remote_P256_Public_Key) {
	...
	tBleStatus status = 0;
	int index_input = 0;
	Osal_MemCpy((void*) &cp0->Remote_P256_Public_Key,
			(const void*) Remote_P256_Public_Key, 64);
	index_input += 64;
	Osal_MemSet(&rq, 0, sizeof(rq));
	rq.ogf = 0x08;
	rq.ocf = 0x026;
...
tBleStatus hci_le_add_device_to_resolving_list(
	...
	index_input += 1;
	Osal_MemCpy((void*) &cp0->Peer_Identity_Address,
			(const void*) Peer_Identity_Address, 6);
	index_input += 6;
...
tBleStatus hci_le_read_peer_resolvable_address(
	...
	if (resp.Status)
		return resp.Status;
	Osal_MemCpy((void*) Peer_Resolvable_Address,
			(const void*) resp.Peer_Resolvable_Address, 6);
	return BLE_STATUS_SUCCESS;
}

tBleStatus hci_le_read_local_resolvable_address(
		uint8_t Peer_Identity_Address_Type,
		const uint8_t *Peer_Identity_Address, uint8_t *Local_Resolvable_Address) {
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	...
	if (resp.Status)
		return resp.Status;
	Osal_MemCpy((void*) Local_Resolvable_Address,
			(const void*) resp.Local_Resolvable_Address, 6);
	return BLE_STATUS_SUCCESS;
}
...
tBleStatus hci_le_enhanced_receiver_test(uint8_t RX_Frequency, uint8_t PHY,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	hci_le_enhanced_receiver_test_cp0 *cp0 =
			(hci_le_enhanced_receiver_test_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
	cp0->RX_Frequency = RX_Frequency;
	index_input += 1;
...
tBleStatus hci_le_enhanced_transmitter_test(uint8_t TX_Frequency,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	hci_le_enhanced_transmitter_test_cp0 *cp0 =
...
tBleStatus hci_le_set_privacy_mode(uint8_t Peer_Identity_Address_Type,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	hci_le_set_privacy_mode_cp0 *cp0 =
			(hci_le_set_privacy_mode_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
	cp0->Peer_Identity_Address_Type = Peer_Identity_Address_Type;
	index_input += 1;
