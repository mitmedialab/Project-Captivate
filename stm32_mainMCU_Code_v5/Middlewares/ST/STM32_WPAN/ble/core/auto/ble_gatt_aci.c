...
tBleStatus aci_gatt_add_service(uint8_t Service_UUID_Type,
	...
	aci_gatt_add_service_cp1 *cp1 = (aci_gatt_add_service_cp1*) (cmd_buffer + 1
	...
	/* var_len_data input */
	{
		...
		Osal_MemCpy((void*) &cp0->Service_UUID, (const void*) Service_UUID,
				size);
		index_input += size;
		{
...
tBleStatus aci_gatt_include_service(uint16_t Service_Handle,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	aci_gatt_include_service_cp0 *cp0 =
			(aci_gatt_include_service_cp0*) (cmd_buffer);
	...
	index_input += 1;
	Osal_MemCpy((void*) &cp0->Include_UUID, (const void*) Include_UUID,
			uuid_size);
...
tBleStatus aci_gatt_add_char(uint16_t Service_Handle, uint8_t Char_UUID_Type,
	...
	aci_gatt_add_char_cp1 *cp1 = (aci_gatt_add_char_cp1*) (cmd_buffer + 2 + 1
...
tBleStatus aci_gatt_add_char_desc(uint16_t Service_Handle, uint16_t Char_Handle,
	...
	aci_gatt_add_char_desc_cp1 *cp1 = (aci_gatt_add_char_desc_cp1*) (cmd_buffer
			+ 2 + 2 + 1
			+ (Char_Desc_Uuid_Type == 1 ?
					2 : (Char_Desc_Uuid_Type == 2 ? 16 : 0)));
	aci_gatt_add_char_desc_cp2 *cp2 = (aci_gatt_add_char_desc_cp2*) (cmd_buffer
			+ 2 + 2 + 1
			+ (Char_Desc_Uuid_Type == 1 ?
					2 : (Char_Desc_Uuid_Type == 2 ? 16 : 0)) + 1 + 1
	...
	/* var_len_data input */
	{
		...
		Osal_MemCpy((void*) &cp0->Char_Desc_Uuid, (const void*) Char_Desc_Uuid,
				size);
		index_input += size;
		{
		...
		index_input += 1;
		Osal_MemCpy((void*) &cp1->Char_Desc_Value,
				(const void*) Char_Desc_Value, Char_Desc_Value_Length);
...
tBleStatus aci_gatt_update_char_value(uint16_t Service_Handle,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	aci_gatt_update_char_value_cp0 *cp0 =
			(aci_gatt_update_char_value_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
	cp0->Service_Handle = Service_Handle;
	index_input += 2;
	...
	index_input += 1;
	Osal_MemCpy((void*) &cp0->Char_Value, (const void*) Char_Value,
			Char_Value_Length);
...
tBleStatus aci_gatt_del_include_service(uint16_t Serv_Handle,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	aci_gatt_del_include_service_cp0 *cp0 =
			(aci_gatt_del_include_service_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
	cp0->Serv_Handle = Serv_Handle;
	index_input += 2;
...
tBleStatus aci_gatt_set_event_mask(uint32_t GATT_Evt_Mask) {
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	aci_gatt_set_event_mask_cp0 *cp0 =
			(aci_gatt_set_event_mask_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
...
tBleStatus aci_gatt_exchange_config(uint16_t Connection_Handle) {
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	aci_gatt_exchange_config_cp0 *cp0 =
			(aci_gatt_exchange_config_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
...
tBleStatus aci_att_find_by_type_value_req(uint16_t Connection_Handle,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	aci_att_find_by_type_value_req_cp0 *cp0 =
			(aci_att_find_by_type_value_req_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
	cp0->Connection_Handle = Connection_Handle;
	index_input += 2;
	...
	index_input += 1;
	Osal_MemCpy((void*) &cp0->Attribute_Val, (const void*) Attribute_Val,
...
tBleStatus aci_att_read_by_type_req(uint16_t Connection_Handle,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	aci_att_read_by_type_req_cp0 *cp0 =
			(aci_att_read_by_type_req_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
...
tBleStatus aci_att_read_by_group_type_req(uint16_t Connection_Handle,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	aci_att_read_by_group_type_req_cp0 *cp0 =
			(aci_att_read_by_group_type_req_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
	int uuid_size = (UUID_Type == 2) ? 16 : 2;
	cp0->Connection_Handle = Connection_Handle;
...
tBleStatus aci_att_prepare_write_req(uint16_t Connection_Handle,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	aci_att_prepare_write_req_cp0 *cp0 =
			(aci_att_prepare_write_req_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
	cp0->Connection_Handle = Connection_Handle;
	index_input += 2;
...
tBleStatus aci_att_execute_write_req(uint16_t Connection_Handle,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	aci_att_execute_write_req_cp0 *cp0 =
			(aci_att_execute_write_req_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
	cp0->Connection_Handle = Connection_Handle;
	index_input += 2;
...
tBleStatus aci_gatt_find_included_services(uint16_t Connection_Handle,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	aci_gatt_find_included_services_cp0 *cp0 =
			(aci_gatt_find_included_services_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
	cp0->Connection_Handle = Connection_Handle;
	index_input += 2;
...
tBleStatus aci_gatt_disc_char_by_uuid(uint16_t Connection_Handle,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	aci_gatt_disc_char_by_uuid_cp0 *cp0 =
			(aci_gatt_disc_char_by_uuid_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
	int uuid_size = (UUID_Type == 2) ? 16 : 2;
	cp0->Connection_Handle = Connection_Handle;
...
tBleStatus aci_gatt_disc_all_char_desc(uint16_t Connection_Handle,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	aci_gatt_disc_all_char_desc_cp0 *cp0 =
			(aci_gatt_disc_all_char_desc_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
	cp0->Connection_Handle = Connection_Handle;
	index_input += 2;
...
tBleStatus aci_gatt_read_char_value(uint16_t Connection_Handle,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	aci_gatt_read_char_value_cp0 *cp0 =
			(aci_gatt_read_char_value_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
	cp0->Connection_Handle = Connection_Handle;
	index_input += 2;
...
tBleStatus aci_gatt_read_using_char_uuid(uint16_t Connection_Handle,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	aci_gatt_read_using_char_uuid_cp0 *cp0 =
			(aci_gatt_read_using_char_uuid_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
	int uuid_size = (UUID_Type == 2) ? 16 : 2;
	cp0->Connection_Handle = Connection_Handle;
...
tBleStatus aci_gatt_read_long_char_value(uint16_t Connection_Handle,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	aci_gatt_read_long_char_value_cp0 *cp0 =
			(aci_gatt_read_long_char_value_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
	cp0->Connection_Handle = Connection_Handle;
	index_input += 2;
...
tBleStatus aci_gatt_write_char_value(uint16_t Connection_Handle,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	aci_gatt_write_char_value_cp0 *cp0 =
			(aci_gatt_write_char_value_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
	cp0->Connection_Handle = Connection_Handle;
	index_input += 2;
...
tBleStatus aci_gatt_write_long_char_value(uint16_t Connection_Handle,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	aci_gatt_write_long_char_value_cp0 *cp0 =
			(aci_gatt_write_long_char_value_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
	cp0->Connection_Handle = Connection_Handle;
	index_input += 2;
...
tBleStatus aci_gatt_write_char_reliable(uint16_t Connection_Handle,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	aci_gatt_write_char_reliable_cp0 *cp0 =
			(aci_gatt_write_char_reliable_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
	cp0->Connection_Handle = Connection_Handle;
	index_input += 2;
...
tBleStatus aci_gatt_write_long_char_desc(uint16_t Connection_Handle,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	aci_gatt_write_long_char_desc_cp0 *cp0 =
			(aci_gatt_write_long_char_desc_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
	cp0->Connection_Handle = Connection_Handle;
	index_input += 2;
...
tBleStatus aci_gatt_read_long_char_desc(uint16_t Connection_Handle,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	aci_gatt_read_long_char_desc_cp0 *cp0 =
			(aci_gatt_read_long_char_desc_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
	cp0->Connection_Handle = Connection_Handle;
	index_input += 2;
...
tBleStatus aci_gatt_write_char_desc(uint16_t Connection_Handle,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	aci_gatt_write_char_desc_cp0 *cp0 =
			(aci_gatt_write_char_desc_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
	cp0->Connection_Handle = Connection_Handle;
	index_input += 2;
...
tBleStatus aci_gatt_read_char_desc(uint16_t Connection_Handle,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	aci_gatt_read_char_desc_cp0 *cp0 =
			(aci_gatt_read_char_desc_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
	cp0->Connection_Handle = Connection_Handle;
	index_input += 2;
...
tBleStatus aci_gatt_write_without_resp(uint16_t Connection_Handle,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	aci_gatt_write_without_resp_cp0 *cp0 =
			(aci_gatt_write_without_resp_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
	cp0->Connection_Handle = Connection_Handle;
	index_input += 2;
...
tBleStatus aci_gatt_confirm_indication(uint16_t Connection_Handle) {
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	aci_gatt_confirm_indication_cp0 *cp0 =
			(aci_gatt_confirm_indication_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
	cp0->Connection_Handle = Connection_Handle;
	index_input += 2;
...
tBleStatus aci_gatt_set_desc_value(uint16_t Serv_Handle, uint16_t Char_Handle,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	aci_gatt_set_desc_value_cp0 *cp0 =
			(aci_gatt_set_desc_value_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
	cp0->Serv_Handle = Serv_Handle;
	index_input += 2;
	...
	index_input += 1;
	Osal_MemCpy((void*) &cp0->Char_Desc_Value, (const void*) Char_Desc_Value,
...
tBleStatus aci_gatt_read_handle_value(uint16_t Attr_Handle, uint16_t Offset,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	aci_gatt_read_handle_value_cp0 *cp0 =
			(aci_gatt_read_handle_value_cp0*) (cmd_buffer);
...
tBleStatus aci_gatt_update_char_value_ext(uint16_t Conn_Handle_To_Notify,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	aci_gatt_update_char_value_ext_cp0 *cp0 =
			(aci_gatt_update_char_value_ext_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
...
tBleStatus aci_gatt_set_access_permission(uint16_t Serv_Handle,
	...
	struct hci_request rq;
	uint8_t cmd_buffer[BLE_CMD_MAX_PARAM_LEN];
	aci_gatt_set_access_permission_cp0 *cp0 =
			(aci_gatt_set_access_permission_cp0*) (cmd_buffer);
	tBleStatus status = 0;
	int index_input = 0;
	cp0->Serv_Handle = Serv_Handle;
	index_input += 2;
