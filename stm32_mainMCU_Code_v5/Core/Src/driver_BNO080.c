...
uint8_t sequenceNumber[6] =
  { 0, 0, 0, 0, 0, 0 }; //There are 6 com channels. Each channel has its own seqnum
...
uint8_t receiveBuffer[I2C_BUFFER_LENGTH] =
  { 0 };
...
bool
IMU_begin (uint8_t deviceAddress, uint8_t intPin, GPIO_TypeDef *intPort)
{
...
float
IMU_getResolution (uint16_t recordID)
{
...
float
IMU_getRange (uint16_t recordID)
{
  //The resolution Q value are 'the same as those used in the sensor's input report'
  //This should be Q1.
  int16_t Q = IMU_getQ1 (recordID);
...
uint32_t
IMU_readFRSword (uint16_t recordID, uint8_t wordNumber)
{
...
void
IMU_frsReadRequest (uint16_t recordID, uint16_t readOffset, uint16_t blockSize)
{
...
bool
IMU_readFRSdata (uint16_t recordID, uint8_t startLocation, uint8_t wordsToRead)
{
  uint8_t spot = 0;
...
float
IMU_qToFloat (int16_t fixedPointValue, uint8_t qPoint)
{
...
void
IMU_enableRotationVector (uint16_t timeBetweenReports)
{
...
//Sends the packet to enable the rotation vector
void
IMU_enableGameRotationVector (uint16_t timeBetweenReports)
{
...
void
IMU_enableAccelerometer (uint16_t timeBetweenReports)
{
...
//Sends the packet to enable the accelerometer
void
IMU_enableLinearAccelerometer (uint16_t timeBetweenReports)
{
...
void
IMU_enableGyro (uint16_t timeBetweenReports)
{
...
//Sends the packet to enable the gyro
void
IMU_enableUncalGyro (uint16_t timeBetweenReports)
{
...
void
IMU_enableMagnetometer (uint16_t timeBetweenReports)
{
...
void
IMU_enableStepCounter (uint16_t timeBetweenReports)
{
...
void
IMU_enableStabilityClassifier (uint16_t timeBetweenReports)
{
...
void
IMU_enableRawAccelerometer (uint16_t timeBetweenReports)
{
...
//Note you must enable basic reporting on the sensor as well
void
IMU_enableRawGyro (uint16_t timeBetweenReports)
{
...
//Note you must enable basic reporting on the sensor as well
void
IMU_enableRawMagnetometer (uint16_t timeBetweenReports)
{
...
void
IMU_enableActivityClassifier (uint16_t timeBetweenReports,
			      uint32_t activitiesToEnable,
			      uint8_t *activityConfidences)
{
...
void
IMU_calibratePlanarAccelerometer ()
{
...
void
IMU_setFeatureCommand_2 (uint8_t reportID, uint16_t timeBetweenReports)
{
...
void
IMU_setFeatureCommand_3 (uint8_t reportID, uint16_t timeBetweenReports,
...
void
IMU_sendCommand (uint8_t command)
{
...
void
IMU_sendCalibrateCommand (uint8_t thingToCalibrate)
{
...
bool
IMU_getData (uint16_t bytesRemaining)
{
...
bool
IMU_sendPacket (uint8_t channelNumber, uint8_t dataLength)
{
