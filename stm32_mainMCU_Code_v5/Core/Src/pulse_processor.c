...
void process_long_pulse(PulseProcessor *self, Pulse *p) {
	if (self->cycle_fix_level_ == kCycleFixNone) {
		...
		if (self->unclassified_long_pulses_.count > 0) {
			Pulse last_long_pulse = vector_pop(
					&self->unclassified_long_pulses_);

			self->time_from_last_long_pulse_ = p->start_time
					- last_long_pulse.start_time;

			int delta = self->time_from_last_long_pulse_
					- (CYCLE_PERIOD - long_pulse_starts[1]);
			if (-LONG_PULSE_STARTS_ACCEPTED_RANGE
					<= delta&& delta < LONG_PULSE_STARTS_ACCEPTED_RANGE) {
	...
	if (self->cycle_fix_level_ >= kCycleFixCandidate) {
		...
		uint16_t time_from_cycle_start = p->start_time
				- self->cycle_start_time_;
		for (int i = 0; i < NUM_BASE_STATIONS; i++) {
...
void process_cycle_fix(PulseProcessor *self) {
	...
	if (self->cycle_long_pulses_[0].count > 0
			|| self->cycle_long_pulses_[1].count > 0) {
		// Increase fix level if we have pulses from both stations.
		if (self->cycle_fix_level_ < kCycleFixMax
				&& self->cycle_long_pulses_[0].count > 0
				&& self->cycle_long_pulses_[1].count > 0) {
		...
		for (int b = 0; b < NUM_BASE_STATIONS; b++) {
			...
			if (num_pulses) {
				uint16_t expected_start_time = self->cycle_start_time_
						+ long_pulse_starts[b];
				for (uint32_t i = 0; i < num_pulses; i++) {
					const Pulse pulse = vector_get(&self->cycle_long_pulses_[b],
							i);
					pulse_start_corrections[b] += pulse.start_time
							- expected_start_time;
		...
		process_pulse_lengths(&self->phase_classifier_, self->cycle_idx_,
				pulse_lens);
	} else {
	...
	if (cycle_phase >= 0) {
		...
		uint16_t base_pulse_start = self->cycle_start_time_
				+ long_pulse_starts[emitting_base]
				+ pulse_start_corrections[emitting_base];
		for (uint32_t i = 0; i < self->cycle_short_pulses_.count; i++) {
			...
			uint16_t pulse_timing = p.start_time + p.pulse_len / 2
					- base_pulse_start;

			// Get longest laser pulse.
			if (SHORT_PULSE_MIN_TIME
					< pulse_timing&& pulse_timing < SHORT_PULSE_MAX_TIME) {
				if (!short_pulses[input_idx]
						|| short_pulses[input_idx]->pulse_len < p.pulse_len) {
		...
		for (uint32_t i = 0; i < self->num_inputs_; i++) {
			if (short_pulses[i]) {
				...
				angles->angles[cycle_phase] =
						(float) M_PI
								* ((int) (short_pulse_timings[i]
										- ANGLE_CENTER_LEN))/ (float)CYCLE_PERIOD;
	...
	if ((cycle_phase >= 0) ? (cycle_phase == 3) : (self->cycle_idx_ % 4 == 0)) {
		...
		self->angles_frame_.fix_level =
				(cycle_phase >= 0 && self->cycle_fix_level_ >= kCycleFixAcquired) ?
