...
BaseStationGeometryDef bs_0 = { { -2.104828, 2.384802, -1.427797 }, { -0.496826,
		0.343897, -0.796805, 0.008548, 0.920031, 0.391751, 0.867808, 0.187821,
		-0.460036 } };

BaseStationGeometryDef bs_1 = { { 1.738303, 2.430314, 0.781285 }, { 0.285992,
		-0.253563, 0.924075, 0.040683, 0.966697, 0.252668, -0.957368, -0.034667,
		0.286784 } };
...
void consume_angles(GeometryBuilder *self, const SensorAnglesFrame *f) {
	...
	if (f->fix_level >= kCycleSynced) {
		...
		if (max_stale < NUM_CYCLE_PHASES * 3) { // We tolerate stale angles up to 2 cycles old.
			float ray1[VEC3D_SIZE], ray2[VEC3D_SIZE], origin1[VEC3D_SIZE],
					origin2[VEC3D_SIZE];
			calc_ray_vec(&self->base_stations_[0], sens->angles[0],
					sens->angles[1], ray1, origin1);
			calc_ray_vec(&self->base_stations_[1], sens->angles[2],
					sens->angles[3], ray2, origin2);

			intersect_lines(origin1, ray1, origin2, ray2, self->vive_vars_.pos,
					&self->vive_vars_.pos_delta);
			...
			osMessageQueuePut(viveQueueHandle, (void*) &self->vive_vars_, NULL,
					0);
...
void calc_ray_vec(const BaseStationGeometryDef *bs, float angle1, float angle2,
		float *res, float *origin) {
	...
	arm_matrix_instance_f32 source_rotation_matrix = { 3, 3,
			(const float*) bs->mat };
	...
	arm_add_f32((const float*) bs->origin, rotated_origin_delta, origin,
			VEC3D_SIZE);
}

uint8_t intersect_lines(float *orig1, float *vec1, float *orig2, float *vec2,
		float *res, float *dist) {
